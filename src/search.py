...
# Version 5 of search
# Added incremental zobrist hash updates
# Added Repetiton Table
...
class Search(BaseSearch):
    ...
    def order_moves(self, board, zobrist_hash, moves, valuator, colour):
        ...
        for i, move in enumerate(moves):
            ...
            new_zobrist_hash = self.zobrist.update_hash(zobrist_hash, board, move)
            position_occurrences = self.RepetitionTable.lookup(new_zobrist_hash)
            board.push(move)
            ...
            elif position_occurrences is not None and position_occurrences > 1:
                scores.append((0, move))
    ...
    def root_negamax(self, start_time, time_limit, valuator, board, zobrist_hash, depth, colour, alpha, beta, current_best_move, current_best_score):
        ...
        ordered_moves = self.order_moves(board, zobrist_hash, moves, valuator, colour)
        ...
        for score, move in ordered_moves:
            ...
            new_zobrist_hash = self.zobrist.update_hash(zobrist_hash, board, move)
            self.RepetitionTable.store(new_zobrist_hash)
            board.push(move)

            value = -self.negamax(start_time, time_limit, valuator, board, new_zobrist_hash, score, depth-1, -colour, -beta, -alpha, beam_width=10, max_depth=depth-1)

            board.pop()
            self.RepetitionTable.remove(new_zobrist_hash)
    ...
    def negamax(self, start_time, time_limit, valuator, board, zobrist_hash, score, depth, colour, alpha=-float('inf'), beta=float('inf'), beam_width=10, max_depth=3):
        ...
        for score, move in ordered_moves:
            ...
            result = self.negamax(start_time, time_limit, valuator, board, new_zobrist_hash, score, depth-1, -colour, -beta, -alpha, beam_width, max_depth)
    ...
    def iterative_deepening(self, valuator, board, zobrist_hash, time_limit, colour, maximum_depth=5):
        """
        ...
        for depth in range(1, maximum_depth + 1):
            ...
            best_move, best_value = self.root_negamax(
                start_time, time_limit, valuator, board, zobrist_hash, depth, colour, -float('inf'), float('inf'), best_move, best_value)
    ...
    def move(self, valuator, board, colour, time_limit):
        ...
        zobrist_hash = self.zobrist.hash_board(board)
        self.RepetitionTable.store(zobrist_hash)
        best_move= self.iterative_deepening(valuator, board, zobrist_hash, time_limit, colour, maximum_depth=10)
        new_zobrist_hash = self.zobrist.update_hash(zobrist_hash, board, best_move)
        self.RepetitionTable.store(new_zobrist_hash)
