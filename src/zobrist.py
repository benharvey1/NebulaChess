...
class ZobristHash():
    ...
    def hash_board(self, board):
        ...
        if board.ep_square:
            ...
            ep_mask = chess.shift_left(ep_mask) | chess.shift_right(ep_mask)    # ep_mask is bitboard containing squares where capturing pawns would be

            # check if there is actually an opponent pawn on these squares
    ...
    def update_hash(self, zobrist_hash, board, move):
        ...
        piece_index = (piece.piece_type - 1)*2 + int(piece.color)
        zobrist_hash ^= self.array[64*piece_index + from_square]

        # add piece to new square
        zobrist_hash ^= self.array[64*piece_index + to_square]
        
        # check for capture
        captured_piece = board.piece_at(to_square)
        if captured_piece is not None:
            captured_index = (captured_piece.piece_type - 1)*2 + int(captured_piece.color)
            zobrist_hash ^= self.array[64*captured_index + to_square]

        # handle castling rights
        if board.has_kingside_castling_rights(chess.WHITE):
            if from_square == chess.E1 or from_square == chess.H1 or (board.is_castling(move) and board.turn == chess.WHITE) or (captured_piece and to_square == chess.H1):
                zobrist_hash ^= self.array[768]

        if board.has_queenside_castling_rights(chess.WHITE):
            if from_square == chess.E1 or from_square == chess.A1 or (board.is_castling(move) and board.turn == chess.WHITE) or (captured_piece and to_square == chess.A1):
                zobrist_hash ^= self.array[769]

        if board.has_kingside_castling_rights(chess.BLACK):
            if from_square == chess.E8 or from_square == chess.H8 or (board.is_castling(move) and board.turn == chess.BLACK) or (captured_piece and to_square == chess.H8):
                zobrist_hash ^= self.array[770]

        if board.has_queenside_castling_rights(chess.BLACK):
            if from_square == chess.E8 or from_square == chess.A8 or (board.is_castling(move) and board.turn == chess.BLACK) or (captured_piece and to_square == chess.A8):
                zobrist_hash ^= self.array[771]

        # handle castling
        if board.is_castling(move):
            if board.turn == chess.WHITE:
                if to_square == chess.G1:  # White short castle
                    zobrist_hash ^= self.array[64 * 7 + chess.H1] 
                    zobrist_hash ^= self.array[64 * 7 + chess.F1]  
                elif to_square == chess.C1:  # White long castle
                    zobrist_hash ^= self.array[64 * 7 + chess.A1] 
                    zobrist_hash ^= self.array[64 * 7 + chess.D1]  

            else:  # Black castling 
                if to_square == chess.G8:  # Black short castle
                    zobrist_hash ^= self.array[64 * 6 + chess.H8]  
                    zobrist_hash ^= self.array[64 * 6 + chess.F8]  
                elif to_square == chess.C8:  # Black long castle
                    zobrist_hash ^= self.array[64 * 6 + chess.A8]  
                    zobrist_hash ^= self.array[64 * 6 + chess.D8] 
            

        # handle en passant possibility in previous game state
        if board.ep_square:
            if board.turn == chess.WHITE:
                ep_mask = chess.shift_down(chess.BB_SQUARES[board.ep_square])
            else:
        ...
        # handle en passant possibility in new game state
        if piece.piece_type == chess.PAWN and abs(from_square - to_square) == 16:
            ep_square = (from_square + to_square) // 2

            if opponent_color == chess.WHITE:
                ep_mask = chess.shift_down(chess.BB_SQUARES[ep_square])
            else:
                ep_mask = chess.shift_up(chess.BB_SQUARES[ep_square])
            ep_mask = chess.shift_left(ep_mask) | chess.shift_right(ep_mask)

            if ep_mask & board.pawns & board.occupied_co[opponent_color]:
                zobrist_hash^= self.array[772 + chess.square_file(ep_square)]

        # handle en passant moves
        if board.is_en_passant(move):
            capture_square = to_square + (-8 if piece.color == chess.WHITE else 8) # captured pawn is row in front
            captured_piece = board.piece_at(capture_square)
            if captured_piece is not None and captured_piece.piece_type == chess.PAWN:
                captured_index = (chess.PAWN - 1)*2 + int(captured_piece.color)
                zobrist_hash ^= self.array[64 * captured_index + capture_square]
           
        # handle promotions
        if move.promotion is not None:
            promoted_index = (move.promotion -1)*2 + int(piece.color)
            zobrist_hash ^= self.array[64 * promoted_index + to_square]
            zobrist_hash ^= self.array[64 * piece_index + to_square]
